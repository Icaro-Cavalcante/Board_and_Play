...
j_venda_repo = RepositoryJogoVenda(db, tb)

def teste_jogo_venda_create():
    '''Teste para o método create da classe JogoVendaRepository.'''
    jogo_venda = JogoVenda("Amigos", "QYI-8625", "jogo_venda", "Lúdico", "Game", 16, 2, 2, 2, 2)
    resultado = j_venda_repo.create(jogo_venda)
    assert resultado == "Jogo venda cadastrado"

def teste_jogo_venda_read():
    '''Teste para o método read da classe JogoVendaRepository'''
    teste_jogo_venda_create()
    resultado = j_venda_repo.read(1)
    assert resultado.categoria == "jogo_venda"

def teste_jogo_venda_update():
    '''Teste para o método update da classe JogoVendaRepository'''
    teste_jogo_venda_create()
