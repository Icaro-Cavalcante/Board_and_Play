import pytest
from unittest.mock import patch
from src.board_and_play_poo.database.database import Database
from src.board_and_play_poo.database.tabelas import Tabela
from src.board_and_play_poo.repositories.repository_produto import RepositoryProduto

tb = Tabela()
db = Database("teste")
tb.create_table(db) # Criando tabelas do db de test
produto_repo = RepositoryProduto(db, tb)

def teste_produto_create():
    '''Teste para o método create de ProdutoRepository'''
    if not produto_repo.read(1):
        tupla = ("Monopoly", "PLO-9641", "jogo_aluguel")
        tupla2 = ("Amigos", "QYI-8625", "jogo_venda")
        resultado = produto_repo.create(tupla)
        produto_repo.create(tupla2)
    else:
        resultado = 1
    assert resultado == produto_repo.read(1)[0]

def teste_produto_read():
    '''Teste para o método read de ProdutoRepository'''
    produto = produto_repo.read(1)
    assert produto.id == 1 

def teste_produto_update():
    '''Teste para o método update de ProdutoRepository'''
    resultado = produto_repo.update(1, "codigo_barras", "PSD-3596")
    assert resultado == "Atributo atualizado." 
 